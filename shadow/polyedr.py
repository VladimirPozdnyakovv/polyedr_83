...
class Edge:
    ...
    def __init__(self, beg, fin):
        ...
        # Центр ребра
        self.mid = (beg + fin) * 0.5
        #  Угол с горизонтальной плоскостью
        v = fin - beg
        self.angle = asin(abs(v.z / v.len))
        # Длина проекции на плоскость
        self.proj = v.len * cos(self.angle)
    ...
    # Сумма проекций невидимых частей
    def shadow_sum(self):
        sum_gaps = sum([(self.r3(x.fin) - self.r3(x.beg)).len
                        for x in self.gaps])
        return self.proj - sum_gaps * cos(self.angle)
    ...
    def sum_spec(self):
        ...
        for e in self.edges:
            for f in self.facets:
                e.shadow(f)
            # Частично видимые
            if e.gaps == [] or e.gaps[0] == Segment(0, 1):
                continue
            # Угол с плоскостью
            if e.angle > pi / 7.0:
                continue
            # Центр вне шара
            if e.mid.x * e.mid.x + e.mid.y * e.mid.y + \
                    e.mid.z * e.mid.z <= 4.0 * self.c * self.c:
                res += e.shadow_sum()
        return res / self.c
